...
export function BatchUpdateGradeCompositionForm({
  ...
  const defaultValues = {
    ...
    examConfigs: exams.map(exam => ({
      type: exam.type,
      examId: exam.id,
      scoreEntryStartDate: exam.scoreEntryStartDate ? new Date(exam.scoreEntryStartDate) : new Date(),
      scoreEntryDeadline: exam.scoreEntryDeadline ? new Date(exam.scoreEntryDeadline) : new Date(),
      scoreCorrectionDeadline: exam.scoreCorrectionDeadline ? new Date(exam.scoreCorrectionDeadline) : new Date(),
    }))
  };
        ...
        {form.watch("examConfigs")?.map((config, index) => (
          <Card key={index} className="mb-4">
            ...
            <CardContent>
              ...
              {(form.formState.errors.examConfigs?.[index]?.scoreEntryStartDate || 
                form.formState.errors.examConfigs?.[index]?.scoreEntryDeadline ||
                form.formState.errors.examConfigs?.[index]?.scoreCorrectionDeadline) && (
                <div className="mt-2 text-destructive text-sm">
                  {form.formState.errors.examConfigs?.[index]?.scoreEntryStartDate?.message || 
                   form.formState.errors.examConfigs?.[index]?.scoreEntryDeadline?.message ||
                   form.formState.errors.examConfigs?.[index]?.scoreCorrectionDeadline?.message}
                </div>
              )}
            </CardContent>
          </Card>
