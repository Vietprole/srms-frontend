...
import { getAllCLOs } from "@/api/api-clo";
...
export default function CLOPage() {
  ...
  const [hocPhanAdd, setHocPhanAdd] = useState(null);
  ...
  const handleOpenAddDialog = async() => {
    const hocKyData = await getAllHocKys();
    setHocKy(hocKyData);
  ...
  const columns = [
    ...
    { width: 200,label: "Tên CLO", dataKey: "tenCLO", align: "center" },
    { width: 400, label: "Mô tả", dataKey: "moTa", align: "center" },
    { width: 200, label: "Thuộc kì", dataKey: "hocKi", align: "center" },
    { width: 300, label: "Thuộc học phần", dataKey: "hocPhan", align: "center" },
  ...
  const fetchData = async () => {
    const allClo= await getAllCLOs();
    const hocPhanData = await getAllHocPhans();
    setHocPhans(hocPhanData);
    setData(allClo);
    setFilteredData(allClo);
  };
                      ...
                      <DialogContent >
                       ...
                       <Autocomplete
                        ...
                        <VirtualizedAutocomplete
                          options={hocPhans}
                          value={hocPhanAdd}
                          onChange={(event, newValue) => setHocPhanAdd(newValue)}
                          getOptionLabel={(option) => `${option.maHocPhan || ""} - ${option.ten || ""}`}
                          label="Chọn học phần"
                          noOptionsText="Không tìm thấy học phần"
                          variant="standard"
                          />
                      </DialogContent>
                      <DialogActions>
                        ...
                        <Button
                          ...
                          variant="contained"
                          color="primary"
                          disabled={!tenCLO || !moTa || !selectedHocKi || !hocPhanAdd} // Disable nếu không có giá trị
                        >
                          Lưu
                        </Button>
                      </DialogActions>
